...
namespace Circular_Area
{
    ...
    public class Circular_Power3_Blend_Inverse : IFilter
    {
        ...
        public Vector2 CircleToSquare(Vector2 input)
        {
            ...
            var x4 = MathF.Pow(x, 4);
            var y4 = MathF.Pow(y, 4);

            var absx = MathF.Abs(x);
            var absy = MathF.Abs(y);

            var sgnxy = (absx * absy) / (x * y);

            var B = Math.Clamp(B_raw, 0.01f, 1);

            if (MathF.Abs(y) < 0.01 || MathF.Abs(x) < 0.01)
            {
            ...
            {
                var circle = new Vector2(
                    ((sgnxy / (MathF.Sqrt(x2 + y2))) * (MathF.Sqrt((1 - MathF.Sqrt(1 - 2 * (3 * B - 2) * x4 * y2 - 2 * (3 * B - 2) * x2 * y4 + 3 * B * (3 * B - 2) * x4 * y4)) / (3 * B - 2)))) * (1 / y),
                    ((sgnxy / (MathF.Sqrt(x2 + y2))) * (MathF.Sqrt((1 - MathF.Sqrt(1 - 2 * (3 * B - 2) * x4 * y2 - 2 * (3 * B - 2) * x2 * y4 + 3 * B * (3 * B - 2) * x4 * y4)) / (3 * B - 2)))) * (1 / x)
                    );
                if ((circle.X >= 0 || circle.X <= 0) && (circle.Y >= 0 || circle.Y <= 0))
                {
                    return new Vector2(
