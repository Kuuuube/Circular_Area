...
{
    [PluginName("Circular Tapered1.5 Mapping Inverse")]
    public class Circular_Tapered1_5_Mapping_Inverse : CircularBase
    {
        public static string Filter_Name = "Circular Tapered1.5 Mapping Inverse";
        ...
        public static Vector2 SquareToCircle(Vector2 input)
        {
            ...
            {
                var circle = new Vector2(
                    (float)((x / Math.Sqrt(x2 + y2)) * ((x2 * y2) + Math.Sqrt(Math.Pow((x2 * y2), 2) - 3 * (x2 * y2) + Math.Pow(Math.Sqrt(x2 + y2), 2)))),
                    (float)((y / Math.Sqrt(x2 + y2)) * ((x2 * y2) + Math.Sqrt(Math.Pow((x2 * y2), 2) - 3 * (x2 * y2) + Math.Pow(Math.Sqrt(x2 + y2), 2))))
                );
