...
namespace Circular_Area
{
    ...
    public class Circular_blended_e_grid_mapping_Inverse : IFilter
    {
        ...
        public Vector2 CircleToSquare(Vector2 input)
        {
            ...
            if (MathF.Abs(y) < 0.1 || MathF.Abs(x) < 0.1)
            {
            ...
            {
                var circle = new Vector2(
                    x * MathF.Sqrt((y2 * (B + 1 - B * y2) - (B + 1 - B * x2) * (B + 1 - B * y2)) / (x2 * y2 - (B + 1 - B * x2) * (B + 1 - B * y2))),
                    y * MathF.Sqrt((x2 * (B + 1 - B * x2) - (B + 1 - B * x2) * (B + 1 - B * y2)) / (x2 * y2 - (B + 1 - B * x2) * (B + 1 - B * y2)))
                    );
                if ((circle.X >= 0 || circle.X <= 0) && (circle.Y >= 0 || circle.Y <= 0))
                {
                    return new Vector2(
