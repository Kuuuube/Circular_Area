...
namespace Circular_Area
{
    ...
    public class Circular_Tapered4_Mapping_Inverse : IFilter
    {
        ...
        public static Vector2 CircleToSquare(Vector2 input)
        {
            ...
            {
                var circle = new Vector2(
                    (sgnxy / (y * MathF.Sqrt(x2 + y2))) * MathF.Sqrt(1 - MathF.Sqrt(1 - 2 * x4 * y2 - 2 * x2 * y4 + 2 * x4 * y4)),
                    (sgnxy / (x * MathF.Sqrt(x2 + y2))) * MathF.Sqrt(1 - MathF.Sqrt(1 - 2 * x4 * y2 - 2 * x2 * y4 + 2 * x4 * y4))
                    );
                if ((circle.X >= 0 || circle.X <= 0) && (circle.Y >= 0 || circle.Y <= 0))
                {
                    return new Vector2(
