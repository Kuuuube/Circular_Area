...
namespace Circular_Area
{
    ...
    public class Circular_3_Squircular_Mapping_Inverse : IFilter
    {
        ...
        public static Vector2 CircleToSquare(Vector2 input)
        {
            ...
            {
                var circle = new Vector2(
                    (sgnxy / y) * MathF.Sqrt((-1 + MathF.Sqrt(1 + 4 * x4 * y2 + 4 * x2 * y4)) / (2 * (x2 + y2))),
                    (sgnxy / x) * MathF.Sqrt((-1 + MathF.Sqrt(1 + 4 * x4 * y2 + 4 * x2 * y4)) / (2 * (x2 + y2)))
                    );
                if ((circle.X >= 0 || circle.X <= 0) && (circle.Y >= 0 || circle.Y <= 0))
                {
                    return new Vector2(
