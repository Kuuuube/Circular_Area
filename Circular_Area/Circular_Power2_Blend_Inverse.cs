...
namespace Circular_Area
{
    ...
    public class Circular_Power2_Blend_Inverse : IFilter
    {
        ...
        public Vector2 CircleToSquare(Vector2 input)
        {
            ...
            {
                var circle = new Vector2(
                    (MathF.Sqrt((x2 + y2 - 2 * B * x2 * y2) / ((x2 + y2) * (1 + x2 * y2 - 2 * B * x2 * y2)))) * x,
                    (MathF.Sqrt((x2 + y2 - 2 * B * x2 * y2) / ((x2 + y2) * (1 + x2 * y2 - 2 * B * x2 * y2)))) * y
                    );
                if ((circle.X >= 0 || circle.X <= 0) && (circle.Y >= 0 || circle.Y <= 0))
                {
                    return new Vector2(
